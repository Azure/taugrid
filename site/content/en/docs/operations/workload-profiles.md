...
status; Tau reads that status and never falls back to a compiled catalog or a
local policy file.
...
Tau accepts a profile set only when all of the following are true:
...
Tau stamps successful output with
...
revision; they are not proof that quota or capacity is still available.
