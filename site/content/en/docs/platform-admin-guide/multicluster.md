...
| Kueue | TauGrid-pinned AKS chart and controller `0.19.2` |
...
There is no separate values file or approval list. The standard installation
supplies controller support only; the platform owner creates worker
identities and routing resources through the steps below.
...
1. Provision a least-privilege worker kubeconfig Secret and protect it from
   tenants; platform owners alone create and copy worker credentials.
...
resolve. AdmissionCheck names and installed controller support alone fall
short of readiness.
...
TauGrid renders them are operator-controlled, out-of-contract paths that rely on
Kueue's own admission control rather than a TauGrid-installed name-based CEL
policy or admission webhook.
...
stay pinned to their original cluster. Do not remove cluster pins from stateful workloads until common
...
| MultiKueue profile fails to reach `Ready` | Inspect its `ExecutionReady` condition and verify that every referenced ClusterQueue uses the ready AdmissionCheck. |
...
authorization blocks new TauGrid submissions while leaving status, logs,
cancellation, resume, and cleanup available for already-existing MultiKueue
workloads.
