...
Use the repository's GPU-enabled AKS Terraform root for a reproducible,
self-service evaluation environment. It creates AKS with managed Entra
authentication, local administrator accounts, OIDC workload identity, a GPU
node pool, and optional ADX integration. It also invokes `tau cluster install`,
so it combines this AKS setup phase with TauGrid setup.

Follow [Provision the platform baseline](../../examples/full-cluster/) for the
supported workflow, its billing and security boundaries, and the canonical
`terraform/aks/README.md` reference. Use remote state with locking for shared
environments. Terraform state, saved plans, and secret-bearing variable files
can contain sensitive values even when terminal output is redacted; keep them
out of the repository and protect them with encrypted, access-controlled
storage.
