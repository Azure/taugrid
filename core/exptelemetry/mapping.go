...
func IsStandardResearchMetric(name string) bool {
	...
	_, known := standardResearchMetrics[lower]
	return known || strings.HasPrefix(lower, "feature/")
}

func ResearchMetricCard(name string) string {
	lower := strings.ToLower(strings.TrimSpace(name))
	switch {
	case lower == "train/return", lower == "eval/score":
		return "Outcome"
	case lower == "train/loss", lower == "eval/loss",
	...
	case strings.HasPrefix(lower, "train/examples"),
		...
		return "Throughput"
	case strings.HasPrefix(lower, "gpu/"), strings.HasPrefix(lower, "system/"):
		return "Systems"
