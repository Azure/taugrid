...
func TestBuildRunHistoryQueryToleratesPreExperimentIDSchema(t *testing.T) {
	query, err := BuildRunHistoryQuery(RunHistoryQueryOptions{})
	if err != nil {
		t.Fatal(err)
	}
	want := "experiment_id=tostring(column_ifexists('experiment_id', ''))"
	if !strings.Contains(query, want) {
		t.Fatalf("run history query must tolerate lifecycle tables without experiment_id using %q:\n%s", want, query)
	}
}
