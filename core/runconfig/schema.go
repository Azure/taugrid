...
const (
	...
	// set, direct job/ray otherwise. These two mark fields the other path
...
var fieldCatalog = map[string]FieldInfo{
	...
	"engine":         {Status: statusSupported, Description: "Selects the dispatch engine when Tau cannot infer it.", Values: []string{"job", "ray"}},
	...
	"run.engine":                     {Status: statusSupported, Description: "Nested dispatch engine selector.", Values: []string{"job", "ray"}},
	...
	"workflow.workload_kind":         {Status: statusSupported, Description: "Workload kind for workflow rendering.", Values: []string{"job", "rayjob", "ray-train", "ray_train"}},
	...
	"runtime.working_dir": {Status: statusDirectOnly, Description: "Clean absolute initial working directory inside the main container. Direct Jobs only; maps to Kubernetes container.workingDir and does not ship local files.", Notes: "This is distinct from Ray-only run.working_dir, which is a host-relative project directory packaged into runtime_env."},
	"runtime.pip":         {Status: statusSupported, Description: "Python packages installed through Ray runtime_env for Ray dispatch."},
	...
	"compute.cpu_request":           {Status: statusSupported, Description: "CPU request for the Job container or per-pod default for direct Ray configs."},
	"compute.memory_request":        {Status: statusSupported, Description: "Memory request for the Job container or per-pod default for direct Ray configs."},
	"compute.cpu_limit":             {Status: statusSupported, Description: "CPU limit for the Job container or per-pod default for direct Ray configs."},
	"compute.memory_limit":          {Status: statusSupported, Description: "Memory limit for the Job container or per-pod default for direct Ray configs."},
	"compute.head_cpu_request":      {Status: statusSupported, Description: "CPU request for the Ray head pod in direct Ray configs."},
	"compute.head_memory_request":   {Status: statusSupported, Description: "Memory request for the Ray head pod in direct Ray configs."},
	"compute.head_cpu_limit":        {Status: statusSupported, Description: "CPU limit for the Ray head pod in direct Ray configs."},
	"compute.head_memory_limit":     {Status: statusSupported, Description: "Memory limit for the Ray head pod in direct Ray configs."},
	"compute.worker_cpu_request":    {Status: statusSupported, Description: "CPU request for Ray worker pods in direct Ray configs."},
	"compute.worker_memory_request": {Status: statusSupported, Description: "Memory request for Ray worker pods in direct Ray configs."},
	"compute.worker_cpu_limit":      {Status: statusSupported, Description: "CPU limit for Ray worker pods in direct Ray configs."},
	"compute.worker_memory_limit":   {Status: statusSupported, Description: "Memory limit for Ray worker pods in direct Ray configs."},
	...
	"execution.launcher":              {Status: statusSupported, Description: "Process launcher, scoped per engine. engine: job accepts python | torchrun (default: python). engine: ray accepts ray-train | ray-tune (default: ray-train).", Values: []string{"python", "torchrun", "ray-train", "ray-tune"}, Notes: "Cross-engine combinations are rejected: torchrun requires engine: job, ray-train/ray-tune require engine: ray."},
