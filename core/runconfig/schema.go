...
var fieldCatalog = map[string]FieldInfo{
	...
	"runtime.rdma":               {Status: statusSupported, Description: "RDMA device injection for InfiniBand workloads. Adds device resources, IPC_LOCK/SYS_RESOURCE/DAC_OVERRIDE capabilities, and a /dev/shm emptyDir volume."},
	...
	"policy.topology":                   {Status: statusSupported, Description: "Explicit placement semantics. TAS-capable GPU queues require one of these values; ResourceFlavors advertise capability but do not choose workload locality.", Values: []string{"unconstrained", "same-host", "same-accelerator-domain", "same-network-domain", "same-site"}, Notes: "same-accelerator-domain selects one provider-declared accelerator island and may span hosts; same-network-domain selects one fabric and may span accelerator domains. Neither guarantees distinct hosts. Missing shared domains remain pending rather than falling back."},
	...
	"metrics.offload.runtime":                  {Status: statusSupported, Description: "Executable contract provided by metrics.offload.image. The standalone collector-v1 runtime is the only supported metrics offloader.", Values: []string{MetricsOffloadRuntimeCollectorV1}, Default: MetricsOffloadRuntimeCollectorV1},
	"metrics.offload.image":                    {Status: statusSupported, Description: "Pinned sidecar image that provides the binary required by metrics.offload.runtime. Use an explicit non-latest tag or @sha256 digest. TAU_METRICS_OFFLOAD_IMAGE overrides this value."},
	...
	"metrics.offload.delivery_mode":            {Status: statusSupported, Description: "Collector sink policy. adx-required delivers only through typed ADX queued ingestion and completes only after a durable ADX receipt.", Values: []string{MetricsOffloadDeliveryADXRequired}, Default: MetricsOffloadDeliveryADXRequired},
	"metrics.offload.adx_cluster_uri":          {Status: statusSupported, Description: "Azure Data Explorer query cluster URI used to derive the queued-ingestion endpoint. Required for metrics offload."},
	"metrics.offload.adx_database":             {Status: statusSupported, Description: "ADX database containing the typed experiment event table. Required for metrics offload."},
	"metrics.offload.adx_table":                {Status: statusSupported, Description: "Typed ADX landing table.", Default: "TauExpMetricEventsV1"},
	"metrics.offload.adx_mapping":              {Status: statusSupported, Description: "Named ADX JSON ingestion mapping.", Default: "TauExpMetricEventsV1Json"},
	"metrics.offload.adx_client_id":            {Status: statusSupported, Description: "User-assigned managed identity client ID used by Azure Workload Identity. Required for metrics offload; secrets are not supported."},
	"metrics.offload.adx_max_attempts":         {Status: statusSupported, Description: "Bounded typed ADX delivery attempts. Zero uses the collector default."},
	"metrics.offload.adx_retry_backoff":        {Status: statusSupported, Description: "Initial typed ADX retry backoff as a positive Go duration. Zero/empty uses the collector default."},
	"metrics.offload.adx_final_status_timeout": {Status: statusSupported, Description: "Maximum wait for ADX final ingestion status as a positive Go duration. Zero/empty uses the collector default."},
