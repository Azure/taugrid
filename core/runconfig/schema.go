...
var fieldCatalog = map[string]FieldInfo{
	...
	"policy.topology":                   {Status: statusSupported, Description: "Explicit placement semantics. TAS-capable GPU queues require one of these values; ResourceFlavors advertise capability but do not choose workload locality. same-network-domain requires one domain but does not independently guarantee InfiniBand or distinct hosts.", Values: []string{"unconstrained", "same-host", "same-network-domain", "same-site"}},
