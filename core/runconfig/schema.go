...
var fieldCatalog = map[string]FieldInfo{
	...
	"metrics.offload.runtime":                  {Status: statusSupported, Description: "Executable contract provided by metrics.offload.image. portal-v1 runs the legacy taugrid-portal command; collector-v1 runs the standalone metrics collector. Tau does not infer this contract from the image repository name.", Values: []string{MetricsOffloadRuntimePortalV1, MetricsOffloadRuntimeCollectorV1}, Default: MetricsOffloadRuntimePortalV1},
	...
	"metrics.offload.delivery_mode":            {Status: statusSupported, Description: "Collector sink policy. remote-write preserves legacy delivery; dual-required requires durable final acknowledgement from remote-write and typed ADX; dual-shadow observes typed ADX failures without blocking completion.", Values: []string{MetricsOffloadDeliveryRemoteWrite, MetricsOffloadDeliveryDualRequired, MetricsOffloadDeliveryDualShadow}, Default: MetricsOffloadDeliveryRemoteWrite},
	"metrics.offload.adx_cluster_uri":          {Status: statusSupported, Description: "Azure Data Explorer query cluster URI used to derive the queued-ingestion endpoint. Required for dual delivery."},
	"metrics.offload.adx_database":             {Status: statusSupported, Description: "ADX database containing the typed experiment event table. Required for dual delivery."},
	...
	"metrics.offload.adx_client_id":            {Status: statusSupported, Description: "User-assigned managed identity client ID used by Azure Workload Identity. Required for dual delivery; secrets are not supported."},
