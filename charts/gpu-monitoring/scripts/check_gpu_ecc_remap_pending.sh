...
# This plugin checks for pending GPU ECC remap operations reported by nvidia-smi.
...
mapfile -t offenders < <(awk -v field="Pending" '
function flush() {
  if (flag) {
    msg = "GPU[" gpu_idx "]"
    if (length(gpu_label)) {
...
if ((${#offenders[@]} == 0)); then
  echo "GPU ECC remap pending: none"
  exit $OK
fi

printf -v details "%s; " "${offenders[@]}"
details=${details%; }
echo "GPU ECC remap pending detected on ${#offenders[@]} GPU(s): $details"
exit $NONOK
