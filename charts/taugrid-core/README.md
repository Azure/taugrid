...
- **Jobs / Queue, Ray, and Cluster Nodes** read the Kubernetes API directly with
  ...
  access to core `services` (Ray dashboard discovery), `pods` and `events` (Job
  detail), core `nodes` (Cluster Nodes hardware inventory), and Kueue
  `localqueues`/`clusterqueues`/`workloads` (queue depth). Without the RBAC,
  those boards return 502 with the API server's forbidden error and the rest of
  the portal still serves; 503 is reserved for a portal that could not build a
