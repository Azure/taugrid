...
function Health() {
  ...
  const query = useBoard<Cluster>('/api/portal/cluster?' + range.api + (instance ? '&instance=' + encodeURIComponent(instance) : ''));
...
function Compute() {
  ...
  return <><p className="muted">Fleet hardware inventory — SKU, agentpool, and CPU/memory/GPU capacity per node, via /api/portal/nodes.</p>
    <Note>Real-time snapshot. Kubernetes node inventory and DaemonSet readiness do not support historical time filtering.</Note>
...
function Utilization() {
  const range = useHistoricalRange('24h');
  const query = useBoard<Cluster>('/api/portal/cluster?' + range.api);
  return <><p className="muted">Per-GPU utilization from GpuHealth() (via /api/portal/cluster), with per-node CPU/memory from node-exporter (via /api/portal/nodeutil) below.</p>
    ...
    }}</BoardResult><NodeUtilization range={range.api}/></>;
}
function NodeUtilization({ range }: { range: string }) {
  const query = useBoard<NodeUtil>('/api/portal/nodeutil?' + range);
