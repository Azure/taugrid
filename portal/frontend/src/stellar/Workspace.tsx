...
import type { ResponseMeta, RunSummary, SeriesPoint } from './contracts';
...
function RunRow({ run, selected, select }: { run: RunSummary; selected: boolean; select: () => void }) {
  return <tr className={selected ? 'selected' : ''}>
    ...
    <td>{run.state}</td><td>{run.owner || '—'}</td><td>{run.updated_at || run.created_at || '—'}</td><td>{run.metric_names.length || '—'}</td>
  </tr>;
}
    ...
    <QueryState name="Run detail" query={query}>{() => {
      ...
      return <><DataState meta={detail}/>
        <dl className="thin-detail"><div><dt>Run ID</dt><dd>{detail.run.run_id}</dd></div><div><dt>State</dt><dd>{detail.run.state}</dd></div>
          <div><dt>Project</dt><dd>{detail.project || state.project || '—'}</dd></div><div><dt>Owner</dt><dd>{detail.run.owner || '—'}</dd></div>
          <div><dt>Started</dt><dd>{detail.started_at || '—'}</dd></div><div><dt>Completed</dt><dd>{detail.completed_at || '—'}</dd></div></dl>
...
function MetricPanel({ experiment, runID, catalog }: { experiment: string; runID: string; catalog: { name: string; unit?: string }[] }) {
  const { state, update } = useExperimentURLState();
  ...
  return <div className="thin-metrics">
    ...
    {!catalog.length ? <Empty>No metrics are available for this run.</Empty> : <>
      <form className="thin-controls metric-controls" onSubmit={event => {
        ...
        <label>Metric<select aria-label="Metric" value={selected} onChange={event => update({ metric: event.target.value })}><option value="">Choose a metric</option>{catalog.map(metric => <option key={metric.name} value={metric.name}>{metric.name}{metric.unit ? ` (${metric.unit})` : ''}</option>)}</select></label>
