...
- Local expstore remains authoritative for run packets, artifacts, checkpoints,
  ...
  state.
- Durable InfiniBand validation follows the same split: bounded
  `TauExpMetrics` rows provide workspace-scoped lifecycle/history discovery,
  while the immutable versioned `core/rdmavalidation` artifact remains
  authoritative for technical evidence. Portal detail reads go through
  workspace-scoped Stellar artifact authorization and never dereference a
  metric-supplied URI directly.
