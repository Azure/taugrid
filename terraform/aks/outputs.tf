...
output "get_credentials_command" {
  description = "Fetch the local AKS administrator kubeconfig with Azure CLI."
  value       = "az aks get-credentials --admin --subscription ${var.subscription_id} --resource-group ${azurerm_resource_group.this.name} --name ${azurerm_kubernetes_cluster.this.name} --overwrite-existing"
}
