...
For a dependency-free first run, use the Ray Tune smoke config:

```bash
tau run --config cli/examples/ray-tune-smoke/tau.yaml --dry-run=client
```
...
- `smoke/debug`: proves a Tau path cheaply; not production data or scale.
...
| --- | --- | --- |
| [`aks-cpu-quickstart`](./aks-cpu-quickstart/) | `operator runbook` | Zero-to-evidence path on a brand-new CPU-only AKS cluster: `az aks create`, `tau cluster install`, a real CPU PyTorch RayJob, and live Stellar metrics. Creates billable Azure resources. Blocked on [#1294](https://github.com/Azure/taugrid/pull/1294) — `runtime.pip` cannot install `torch` yet. |
| [`aks-gpu-quickstart`](./aks-gpu-quickstart/) | `operator runbook` | GPU counterpart to `aks-cpu-quickstart` on a single A100 node, including the AKS device-plugin and MIG gotchas. Proves CUDA execution with evidence a CPU cannot fabricate. Creates expensive billable Azure resources. Blocked on [#1294](https://github.com/Azure/taugrid/pull/1294) — `runtime.pip` cannot install `torch` yet. |
...
| [`kind-smoke`](./kind-smoke/) | `smoke/debug` | Prove the local Tau -> Kueue -> Kubernetes Job path on a disposable Kind cluster before trying AKS/GPU-specific prepare and storage flows. |
...
| [`ray-tune-smoke`](./ray-tune-smoke/) | `smoke/debug` | Minimal Ray Tune HPO path: the researcher supplies `train_func(config)` and a search space, and Tau generates the Tuner/TorchTrainer wrapper. |
