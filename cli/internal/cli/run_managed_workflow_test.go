...
// GPU demand. A training RayJob renders one GPU head plus (workers-1) GPU
// worker replicas; passing the per-pod count let an inadmissible job clear the
// preflight and then sit Pending in Kueue with no explanation.
func TestManagedWorkflowGPUDemandCountsEveryGPUPod(t *testing.T) {
	for _, tc := range []struct {
		...
		// Eval renders exactly one GPU pod; its workers are CPU-only.
