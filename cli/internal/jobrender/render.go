...
func buildJob(p profile.Profile, o Options, image string, cmd []string, extraEnv map[string]string, topologyPlan topology.Plan, storagePlan storagePlan, gpu profile.GPUContract) (map[string]any, error) {
	...
	grace := effectiveTerminationGracePeriodSeconds(o)
	...
	if o.ExecutionDeadlineSeconds > 0 {
		...
		if p.ActiveDeadlineSeconds > 0 && p.ActiveDeadlineSeconds < activeSeconds {
			activeSeconds = p.ActiveDeadlineSeconds
		}
...
func effectiveTerminationGracePeriodSeconds(o Options) int64 {
	grace := int64(600)
