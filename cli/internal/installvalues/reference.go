...
var catalog = []struct {
	...
	{"baselineQueue.enabled", fieldInfo{"bool", "true", "Create the baseline ClusterQueue, LocalQueue, and ResourceFlavors"}},
	...
	{"baselineQueue.topology.enabled", fieldInfo{"bool", "true", "Reference the controller-owned Topology when GPU Sites are configured"}},
	...
	{"baselineQueue.gpu.flavors", fieldInfo{"list", "taugrid-default-gpu-topology (generic)", "GPU ResourceFlavors and per-flavor quotas; replace the generic flavor with exact class-labeled flavors when hardware is known"}},
	...
	{"tau-core-controller.tauCluster.sites", fieldInfo{"list", "[]", "Microsoft and Flex GPU Sites used to reconcile network-domain labels and the TauGrid Kueue Topology"}},
